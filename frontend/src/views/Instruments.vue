<template>
  <div>
    <div class="container-fluid mt-4">
      <h1 class="h1">Instruments</h1>
      <b-alert :show="isLoading" variant="info">Loading...</b-alert>
      <b-row>
        <b-col>
          <table class="table table-striped">
            <thead>
              <tr>
                <th>ID</th>
                <th>name</th>
                <th>model</th>
                <th>&nbsp;</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="instrument in instruments" :key="instrument.id">
                <td>{{ instrument.id }}</td>
                <td>{{ instrument.name }}</td>
                <td>{{ instrument.model }}</td>
                <td class="text-right">
                  <a href="#" @click.prevent="populateInstrumentToEdit(instrument)">Edit</a>
                  -
                  <a href="#" @click.prevent="deleteInstrument(instrument.id)">Delete</a>
                </td>
              </tr>
            </tbody>
          </table>
        </b-col>
        <b-col lg="3">
          <b-card :title="model.id ? 'Edit Instrument ID#' + model.id : 'New Instrument'">
            <form @submit.prevent="saveInstrument">
              <b-form-group label="name">
                <b-form-input
                  v-model="model.name"
                  type="text"
                ></b-form-input>
              </b-form-group>
              <b-form-group label="model">
                <b-form-textarea
                  v-model="model.model"
                  rows="4"
                ></b-form-textarea>
              </b-form-group>
              <div>
                <b-btn type="submit" variant="success">Save Instrument</b-btn>
              </div>
            </form>
          </b-card>
          <div v-if="errors">
            <span v-for="error in errors" :key="error">
              {{ error }}
            </span>
          </div>
        </b-col>
      </b-row>
    </div>
  </div>
</template>

<script lang="ts">
import { Component, Vue } from 'vue-property-decorator'

import { backend, Instrument } from '../backend'

const NO_INSTRUMENT = { id: '', name: '', model: '' }

@Component
export default class Home extends Vue {
  isLoading: Boolean = false
  instruments: Array<Instrument> = []
  model: Instrument = NO_INSTRUMENT
  error: Object = null
  errors: Array<String> = []

  async beforeMount() {
    this.refreshInstruments()
  }

  async refreshInstruments() {
    this.isLoading = true
    try {
      let response = await backend.getInstruments()
      this.instruments = response.data
    } catch (err) {
      this.parseError(err)
    }
    this.isLoading = false
  }

  async populateInstrumentToEdit(instrument) {
    this.model = Object.assign({}, instrument)
  }

  async saveInstrument() {
    try {
      if (this.model.id) {
        await backend.updateInstrument(this.model.id, this.model)
      } else {
        await backend.createInstrument(this.model)
      }
      this.model = NO_INSTRUMENT // reset form
      await this.refreshInstruments()
    } catch (err) {
      this.parseError(err)
    }
  }

  async deleteInstrument(id) {
    if (confirm('Are you sure you want to delete this instrument?')) {
      // if we are editing a instruments we deleted, remove it from the form
      if (this.model.id === id) {
        this.model = NO_INSTRUMENT
      }
      await backend.deleteInstrument(id)
      await this.refreshInstruments()
    }
  }

  parseError(error) {
    this.error = error
    this.errors = []
    if (error) {
      for (let idx in error.response.data.errors) {
        this.errors.push(idx + ': ' + error.response.data.errors[idx])
      }
    }
  }
}
</script>
